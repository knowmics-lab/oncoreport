...
@singleton()
export default class Location implements Adapter<ResourceObject> {
  ...
  public async fetchOne(
    ...
    >(`locations/${realId}`);
    if (!result.data) throw new ApiError('Unable to fetch the location');
  ...
  public async fetchPage(): Promise<Collection<ResourceObject>> {
    ...
    if (!result.data) throw new ApiError('Unable to fetch locations');
