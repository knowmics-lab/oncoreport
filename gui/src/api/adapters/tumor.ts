...
@singleton()
export default class Tumor implements Adapter<ResourceObject> {
  ...
  public async fetchOne(
    ...
    >(`tumors/${realId}`);
    if (!result.data) throw new ApiError('Unable to fetch the tumor');
  ...
  public async fetchPage(): Promise<Collection<ResourceObject>> {
    ...
    if (!result.data) throw new ApiError('Unable to fetch tumors');
