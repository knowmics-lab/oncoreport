...
@singleton()
export default class Pathology implements Adapter<ResourceObject> {
  ...
  public async fetchOne(
    ...
    >(`diseases/${realId}`);
    if (!result.data) throw new ApiError('Unable to fetch the patology');
  ...
  public async fetchPage(): Promise<Collection<ResourceObject>> {
    ...
    if (!result.data) throw new ApiError('Unable to fetch pathology');
