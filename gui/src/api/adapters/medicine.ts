...
@singleton()
export default class Medicine implements Adapter<ResourceObject> {
  ...
  public async fetchOne(
    ...
    >(`medicines/${realId}`);
    if (!result.data) throw new ApiError('Unable to fetch the medicine');
  ...
  public async fetchPage(): Promise<Collection<ResourceObject>> {
    ...
    if (!result.data) throw new ApiError('Unable to fetch medicines');
