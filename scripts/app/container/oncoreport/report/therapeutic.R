...
therapeutic.references <- read.csv(paste0(path_project, "/txt/reference/", pt_fastq, ".txt"),
  sep = "\t",
...
therapeutic.trials <- read.csv(paste0(path_project, "/txt/trial/", pt_fastq, ".txt"),
  sep = "\t",
  stringsAsFactors = FALSE
)
list.all.trials <- unique(therapeutic.trials$Clinical_trial)

all.annotations <- read.csv(paste0(path_project, "/txt/", pt_fastq, "_definitive.txt"),
  sep = "\t",
  colClasses = "character", stringsAsFactors = FALSE
)
all.annotations$Disease[is.na(all.annotations$Disease)] <- "Unspecified"
all.annotations <- diseases_db_simple %>% right_join(all.annotations, by = "Disease", relationship = "many-to-many")
...
if (nrow(primary.annotations) > 0) {
  evidence.groups <- list(
    clinical.impact = .clinical_impact_evidences,
    others = .other_impact_evidences
  )
  ...
  evidence.groups <- suppressMessages(lapply(evidence.groups, function(eg) {
    ...
    e.annotations <- e.annotations[, c(
      "Gene", "Variant", "Drug", "Evidence_type", "Clinical_significance",
    ...
    e.annotations <- e.annotations %>%
      ...
      summarise(
        Evidence_statement = paste0("<li>", Evidence_statement, "</li>", collapse = ""),
  ...
  template.env$therapeutic_indications$summary <- suppressMessages(lapply(evidence.groups, function(eg) {
    es <- eg[, c(
      "Gene", "Variant", "Drug", "Evidence_type", "Clinical_significance", "Evidence_level", "Type",
      "Evidence_statement", "Reference", "Score", "AIFA", "EMA", "FDA", "year", "id"
    ), drop = FALSE]
    ...
    es <- es[, c(
      "Evidence", "Gene", "Variant", "Drug", "Evidence_type", "Clinical_significance", "Evidence_level",
    ...
    names(es) <- c(
      "#", "Gene", "Variant", "Drug", "Evidence Type", "Clinical Significance", "Evidence Level", "Type",
    ...
    if (is.null(sorted_evidences)) {
      sorted_evidences <<- es
    } else {
      sorted_evidences <<- rbind(sorted_evidences, es)
    }
    ...
    table <- gsub("{{APPROVED}}", "&#9989;",
      gsub("{{NOTAPPROVED}}", "&#10060;", as.character(table), fixed = TRUE),
      fixed = TRUE
    )
    return(table)
  }))
  ...
  evidence.annotations <- evidence.annotations %>%
    group_by(Gene, Evidence_statement, References) %>%
    summarize(
      Evidence = str_c(Evidence, collapse = ", "), Less = str_c(Less, collapse = ", "),
