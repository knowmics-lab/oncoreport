...
offlabel.references <- read.csv(paste0(path_project, "/txt/reference/", pt_fastq, "_off.txt"),
  sep = "\t",
  stringsAsFactors = FALSE, colClasses = "character"
) %>%
  select(PMID, Reference) %>%
  distinct(PMID, .keep_all = TRUE)

offlabel.trials <- read.csv(paste0(path_project, "/txt/trial/", pt_fastq, ".txt"),
  sep = "\t",
...
if (nrow(offlabel.annotations) > 0) {
  ...
  e.annotations <- offlabel.annotations[, c(
    "Disease", "Gene", "Variant", "Drug", "Evidence_type", "Clinical_significance",
  ...
  es <- es[, c(
    "Evidence", "Disease", "Gene", "Variant", "Drug", "Evidence_type", "Clinical_significance", "Evidence_level",
  ...
  names(es) <- c(
    "#", "Disease", "Gene", "Variant", "Drug", "Evidence Type", "Clinical Significance", "Evidence Level",
  ...
  evidence.annotations <- evidence.annotations %>%
    group_by(Disease, Gene, Evidence_statement, References) %>%
    summarize(
      Evidence = str_c(Evidence, collapse = ", "),
