...
id_evidence <- 0
pharmgkb_references <- read.csv(paste0(path_project, "/txt/reference/", pt_fastq, "_pharm.txt"),
  sep = "\t",
  colClasses = "character", stringsAsFactors = FALSE
)
pharmgkb_references <- pharmgkb_references[, c("PMID", "Reference")]

pharmgkb_annot <- read.csv(paste0(path_project, "/txt/", pt_fastq, "_pharm.txt"),
  sep = "\t",
  colClasses = "character", stringsAsFactors = FALSE
)
...
pharmgkb_annot <- pharmgkb_annot %>%
  inner_join(pharmgkb_references, relationship = "many-to-many") %>%
...
if (nrow(pharmgkb_annot) > 0) {
  ...
  pharmgkb_mutations$Details <- paste0(
    '[<a href="Javascript:;" class="evidence-details-link" data-id="#det-',
  ...
  pharmgkb_mutations <- pharmgkb_mutations[, c(
    "Evidence", "Gene", "Variant", "Drug", "Clinical_significance", "Type",
    "Details", "Reference"
  )]
  ...
  pharmgkb_references$Less <- paste0(
    '[<a href="Javascript:;" class="evidence-summary-link" data-id="#mut-',
...
mut_details <- read.csv(paste0(path_project, "/txt/", pt_fastq, "_pharm.txt"),
  sep = "\t",
  colClasses = "character", stringsAsFactors = FALSE
)
