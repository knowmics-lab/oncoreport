...
write.table(civic2,file="./civic_database.txt", quote=FALSE,
            row.names = FALSE, na= "NA", sep = "\t")

#civic
civic <- read.csv("./civic_database.txt", sep="\t")
civic$chromosome <- paste("chr",civic$chromosome, sep = "")
civic$reference_bases <- NULL
civic$variant_bases <- NULL
civic$gene <- NULL
civic$variant <- NULL
civic$disease <- NULL
civic$drugs <- NULL
civic$drug_interaction_type <- NULL
civic$evidence_type <- NULL
civic$evidence_level <- NULL
civic$evidence_direction <- NULL
civic$clinical_significance <- NULL
civic$evidence_statement <- NULL
civic$variant_summary <- NULL
civic$citation_id <- NULL
civic$citation <- NULL
civic$code <- rownames(civic)
write.table(civic, "./civic_bed.bed", sep="\t", col.names = FALSE, row.names = FALSE, quote=FALSE)
...
library(data.table)
library(dplyr)
a <- fread("/Cosmic_downloads/CosmicResistanceMutations_hg38.txt")
b <- fread("/Cosmic_downloads/CosmicCodMutDef_hg38.txt") 
colnames(a)[8] <- "ID"
...
write.table(c,"./cosmic_databasehg38.txt" , sep="\t", quote=FALSE,
            row.names=FALSE, col.names=TRUE, na="NA")

###############################################################################################
#Refgene
ref <- read.csv("./ncbiRefSeq_hg38.txt", sep="\t", header=FALSE)
colnames(ref)[1] <- "bin"
colnames(ref)[2] <- "name"
...
write.table(e1,"./refgene_databasehg38.txt",
