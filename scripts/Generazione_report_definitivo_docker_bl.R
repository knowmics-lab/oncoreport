...
```{r Patient Information, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE}
##Patients information
library(knitr)
library(kableExtra)
options(knitr.table.format = "html")
args=commandArgs(trailingOnly = TRUE)
pat<-data.frame(Name=args[1],Surname= args[2], ID=args[3],Gender=args[4], Age=args[5], Sample="Liquid Biopsy", Tumor=args[6] )
kable(pat) %>%
...
```{r Detected variant therapeutic benefit, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
...
n<-subset(n, select= c(Variant, Drug, Drug_interaction_type, Evidence_type, Clinical_significance, Evidence_level, Type, Reference))
row.names(n)<-NULL
if(nrow(n)>1){
...
### Details
```{r Detected variant therapeutic benefit details, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
##Mutations information
library(knitr)
library(kableExtra)
options(knitr.table.format = "html")
...
args=commandArgs(trailingOnly = TRUE)
try({urls<-read.csv(paste0("/Trial/", args[7],".txt"), sep= "\t")
urls <- urls[!(urls$Drug == ""), ]
urls <- urls[!(urls$Drug == " "), ]
...
options(knitr.table.format = "html")
args=commandArgs(trailingOnly = TRUE)
try({x<-read.csv(paste0("/pharm/results/",args[7],"/", args[7],".txt"), sep= "\t")
x <- sapply(x, as.character)
x[is.na(x)] <- " "
x <- as.data.frame(x)
x$Drug <- as.character(x$Drug , levels=(x$Drug))
...
for (n in xa){
    if (dim(n)[1]!=0){
      cat("  \n#### Gene",  as.character(n$Gene)[1], " \n")
      n$Drug  <- as.character(n$Drug , levels =(n$Drug))
n<-subset(n, select= c(Variant, Drug, Clinical_significance, Type, Reference))
row.names(n)<-NULL
if(length(n$Drug)>1){
for(i in 1:(length(n$Drug)-1)){
...
library(kableExtra)
args=commandArgs(trailingOnly = TRUE)
try({xref<-read.csv(paste0("/refgene/",args[7],".txt"), sep= "\t")
xclin<-read.csv(paste0("/clinvar/",args[7],".txt"), sep="\t")
xcr<-merge(xref,xclin, all.x=TRUE)
xcr <- format.data.frame(xcr, digits = NULL, na.encode = FALSE)
xcr[is.na(xcr)] <- " "
names(xcr)[6]<- "Gene"
names(xcr)[7]<- "Change Type"
names(xcr)[8]<- "Clinical significance"
names(xcr)[2]<- "Position"
xcr<-subset(xcr, select= c("Gene", "Chromosome", "Position", "Ref_base", "Var_base", "Change Type", "Clinical significance", "Type"))
rownames(xcr)<-NULL
kable(xcr) %>%
...
s<- gsub(",([A-Za-z])", ", \\1", n$Drug)
n["Drug"]<-s
n<-subset(n, select= c(Gene,Variant, Drug, Drug_interaction_type, Evidence_type, Clinical_significance, Evidence_level, Disease, Type, Reference))
df <- data.frame(n)
row.names(df)<-NULL
...
df_total1<-subset(df_total, select= c(Gene,Variant, Drug, Evidence_type, Clinical_significance, Evidence_level, Type, Reference))
t <- kable(df_total1) %>%
kable_styling(bootstrap_options = c("striped","hover","responsive"))
...
```{r echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
args=commandArgs(trailingOnly = TRUE)
try({x_url<-read.csv(paste0("/Trial/", args[7],"_off.txt"), sep= "\t")
x_url <- x_url[!(x_url$Drug == ""), ]
library(dplyr)
library(knitr)
library(kableExtra)
x_url <- x_url[order(x_url[,2]),]
x_url$Reference <- paste0(x_url$Reference,"a")
link1<-subset(x_url, select= c(Gene, Variant, Clinical_trial, Reference))
t <- link1 %>%
  mutate(Clinical_trial = cell_spec(x_url$Drug, "html", link = Clinical_trial)) %>%
  kable("html", escape = FALSE) %>%
kable_styling(bootstrap_options = c("hover", "condensed"))
...
options(knitr.table.format = "html")
args=commandArgs(trailingOnly = TRUE)
try({cosm<-read.csv(paste0("/cosmic/results/",args[7],"/",args[7],".txt"), sep= "\t")
attach(cosm)
...
options(knitr.table.format = "html")
args=commandArgs(trailingOnly = TRUE)
try({x<-read.csv(paste0("/cosmic/results/",args[7],"/", args[7],".txt"), sep= "\t")
x$Var_base <- as.character(x$Var_base)
x$Ref_base<-as.character(x$Ref_base)
  for(k in 1:length(x$Var_base)){
    if (is.na(x$Var_base[k])){
...
x$Disease <- gsub(x$Disease, pattern="��", replace="�") #
