...
while getopts "n:s:i:g:a:t:p:h:b:c:" OPTION; do
  case "${OPTION}" in
    n)
      name=$OPTARG
    ...
    *)
    ...
    /? | h)
      ...
      if [[ -z "$prep_databases" ]] || [[ -z "$name" ]] || [[ -z "$surname" ]] || [[ -z "$tumor" ]] || [[ -z "$age" ]] || [[ -z "$index" ]] || [[ -z "$gender" ]] || [[ -z "$id" ]] || [[ -z "$threads" ]]; then
...
PATH_TRIM_TUMOR=trim_tumor
PATH_TRIM_BLOOD=trim_blood
PATH_SAM_TUMOR=sam_tumor
PATH_SAM_BLOOD=sam_blood
PATH_BAM_ANNO_TUMOR=bam_annotato_tumor
PATH_BAM_ANNO_BLOOD=bam_annotato_blood
PATH_BAM_SORT_TUMOR=bam_sortato_tumor
PATH_BAM_SORT_BLOOD=bam_sortato_blood
PATH_BAM_ORD_TUMOR=bam_ordinato_tumor
PATH_BAM_ORD_BLOOD=bam_ordinato_blood
PATH_MARK_DUP_TUMOR=mark_dup_tumor
PATH_MARK_DUP_BLOOD=mark_dup_blood
PATH_VCF_MUT=mutect
...
do
    ...
    echo "Report creation"
		...
		Rscript report_definitivo_docker_tmVSnm.R $FASTQ_NAME_T "$tumor"
