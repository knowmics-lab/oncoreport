...
for (m in files_definitivi) {
  ...
  if(dim(x)[1]!=0){
    ...
    write.table(urls, paste0(args[3], "/Trial/",tools::file_path_sans_ext(basename(m)) ,".txt"), quote=FALSE,
...
for (m in files_definitivi) {
  ...
  if(dim(x)[1]!=0){
    ...
    write.table(urls, paste0(args[3], "/Reference/",tools::file_path_sans_ext(basename(m)),".txt"), quote=FALSE,
                row.names = FALSE, na= "NA", sep = "\t")
  }}
}, silent = TRUE)
