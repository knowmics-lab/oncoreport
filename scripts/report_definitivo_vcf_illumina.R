  ...
  for (m in files_cgi_results){
    if (basename(i) == basename(m)){
      ...
      if(dim(cgi)[1]!=0 && dim(civic[1])!=0){
        ...
        disease(civic, 19)
        e1 <- e1[,c("Chromosome","Start", "Stop", "Ref_base", "Var_base", "Gene", "Variant", "Drug", "Drug_interaction_type","Evidence_level", "Evidence_type", " Clinical_significance", "Evidence_direction", "Disease", "Variant_summary", "Database", "Citation", "PMID", "Type", "Evidence_statement")]
        definitive(17, 20, 19)
        write.table(e4, paste0(args[3], "/definitive/", tools::file_path_sans_ext(basename(i)) ,".txt"),
                    quote=FALSE, row.names = FALSE, na= "NA", sep = "\t")
      } else if (dim(cgi)[1]!=0 && dim(civic[1])==0){
...
for(i in files_results){
  x<-read.csv(i, sep="\t")
  if(nrow(x)!=0){
    x1 <- x[,c("Database", "Chromosome","Start", "Stop", "Ref_base", "Var_base", "Gene", "Type", "ID", "Drug",
