...
class EsmoParser extends Command
{
    ...
    {
        $disease = strtolower(mb_convert_encoding($disease, 'ASCII'));
        $scoredGuidelines = array_filter(
            array_map(function ($d) use ($disease) {
                $title = strtolower(mb_convert_encoding($this->handleAbbreviation($d['guidelineName']), 'ASCII'));
            ...
            static fn($d) => str_contains(strtolower($d[1]), $disease) || $d[2] > 75
        );
